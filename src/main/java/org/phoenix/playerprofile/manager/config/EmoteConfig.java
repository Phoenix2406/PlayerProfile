...
public class EmoteConfig {
    ...
    private static YamlDocument configData;
    private final String configName;
    private final boolean setAutoUpdate;
    private final String versioningPath;

    public EmoteConfig(ProfileCore plugin, String configName, boolean setAutoUpdate, String versioningPath) {
        this.plugin = plugin;
    ...
    public void load() {
        try {
            configData = YamlDocument.create(new File(plugin.getDataFolder(), configName), plugin.getResource(configName), GeneralSettings.DEFAULT, LoaderSettings.builder().setAutoUpdate(setAutoUpdate).build(), DumperSettings.DEFAULT, UpdaterSettings.builder().setVersioning(new BasicVersioning(versioningPath)).build());
