...
public class SoundConfig {
    ...
    private static YamlDocument soundConfig;
    private final String configName;
    private final boolean setAutoUpdate;
    private final String versioningPath;

    public SoundConfig(ProfileCore plugin, String configName, boolean setAutoUpdate, String versioningPath) {
        this.plugin = plugin;
    ...
    public void load() {
        try {
            soundConfig = YamlDocument.create(new File(plugin.getDataFolder(), configName), plugin.getResource(configName), GeneralSettings.DEFAULT, LoaderSettings.builder().setAutoUpdate(setAutoUpdate).build(), DumperSettings.DEFAULT, UpdaterSettings.builder().setVersioning(new BasicVersioning(versioningPath)).build());
