...
public class MythicLibHook {
    ...
    public MythicLibHook() {
        if (Bukkit.getPluginManager().getPlugin("MythicLib") != null) {
            Logger.log(Logger.LogLevel.SUCCESS, "Hook into MythicLib.");
        } else {
            throw new UnknownDependencyException("Couldn't find MythicLib Plugin! This plugin is required as MMOItems and MMOCore dependency.");
        }
    }
}
