...
public class CommandInfo {
    ...
    for Player Profile info command on Discord
    */
    @JDACommand(name = "info", description = "See information about this plugin via discord.", adminPerms = "Permission.Admin", onlyAdmin = true)
    public void onInfoCommandUsed(SlashCommandInteractionEvent event) {
        ...
        if (DiscordCore.getDiscordCore().getGuild().getVoiceChannelById(ProfileCore.getDiscordConfig().getInstance().getString("ChannelId")) == null) {
            if (DiscordCore.getDiscordCore().getGuild().getTextChannelById(ProfileCore.getDiscordConfig().getInstance().getString("ChannelId")) == null) {
                CommandConstruction.InfoCommand(event);
            } else {
