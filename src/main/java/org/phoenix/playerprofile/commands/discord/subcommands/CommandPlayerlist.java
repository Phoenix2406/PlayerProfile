...
public class CommandPlayerlist {
    ...
    for Player Profile playerlist command on Discord
    */
    @JDACommand(name = "playerlist", description = "See all online players in-game.", memberPerms = "Permission.Member")
    public void onPlayerlistCommandUsed(SlashCommandInteractionEvent event) {
        ...
        if (DiscordCore.getDiscordCore().getGuild().getVoiceChannelById(ProfileCore.getDiscordConfig().getInstance().getString("ChannelId")) == null) {
            if (DiscordCore.getDiscordCore().getGuild().getTextChannelById(ProfileCore.getDiscordConfig().getInstance().getString("ChannelId")) == null) {
                CommandConstruction.PlayerlistCommand(event);
            } else {
